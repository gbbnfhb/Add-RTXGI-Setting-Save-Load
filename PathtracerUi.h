...
// UIの状態を保持する構造体 (既存の定義)
struct PathtracerUIState
{
    // ... (buildUIで使われているメンバー変数がすべて定義されていると仮定)
    bool showUI = true;
    ...
    enum class TechSelection { None,Nrc,Sharc };
    TechSelection techSelection = TechSelection::None;

    enum class DenoiserSelection { None,Accumulation,Nrd };
    DenoiserSelection denoiserSelection = DenoiserSelection::None;

    int bouncesMax = 8;
    int samplesPerPixel = 1;
    float exposureAdjustment = 0.0f;
    // ... buildUIで使われている他のすべてのメンバー変数をここに記述
};
