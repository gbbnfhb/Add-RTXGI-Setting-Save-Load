...
#endif
#include <iostream>
#include "ImGuiFileBrowser.h"   //https://github.com/gallickgunner/ImGui-Addons
...
void PathtracerUI::buildUI(void)
{
    ...
    // --- ファイルを開く処理 ---
    bool load_file_dialog_flag = false,save_file_dialog_flag = false;

    if(ImGui::Button("Load Settings..."))
    {
        // ボタンが押されたらフラグを立てるだけ
        load_file_dialog_flag = true;
    }
    // 10ピクセル分の間隔を空けて、同じ行に配置
    ImGui::SameLine(0.0f,10.0f);

    if(ImGui::Button("Save Settings..."))
    {
        // ボタンが押されたらフラグを立てるだけ
        save_file_dialog_flag = true;
    }
    ...
    if(load_file_dialog_flag)
        ImGui::OpenPopup("Open File");
    if(save_file_dialog_flag)
    ...
    if(file_dialog.showFileDialog("Open File",imgui_addons::ImGuiFileBrowser::DialogMode::OPEN,ImVec2(700,310),".json,*.*"))
    {
        ...
        LoadSettings(file_dialog.selected_path);
    }
    if(file_dialog.showFileDialog("Save File",imgui_addons::ImGuiFileBrowser::DialogMode::SAVE,ImVec2(700,310),".json,*.*"))
    {
        ...
        SaveSettings(file_dialog.selected_path);
    }
